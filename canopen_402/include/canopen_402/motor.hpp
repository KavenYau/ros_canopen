...
class Command402 {
  ...
  class TransitionTable {
    boost::container::flat_map<
        std::pair<State402::InternalState, State402::InternalState>, Op>
        transitions_;
    void add(const State402::InternalState &from,
             const State402::InternalState &to, Op op) {
      transitions_.insert(std::make_pair(std::make_pair(from, to), op));
    }

  public:
    TransitionTable();
    const Op &get(const State402::InternalState &from,
  ...
  static State402::InternalState
  nextStateForEnabling(State402::InternalState state);
  Command402();
...
public:
  ...
  static bool setTransition(uint16_t &cw, const State402::InternalState &from,
...
public:
  ...
  typedef WordAccessor<(1 << Command402::CW_Operation_mode_specific0) |
...
template <uint16_t ID, typename TYPE, uint16_t OBJ, uint8_t SUB,
          uint16_t CW_MASK>
...
typedef ModeForwardHelper<MotorBase::Profiled_Velocity, int32_t, 0x60FF, 0, 0>
    ProfiledVelocityMode;
typedef ModeForwardHelper<MotorBase::Profiled_Torque, int16_t, 0x6071, 0, 0>
    ProfiledTorqueMode;
typedef ModeForwardHelper<MotorBase::Cyclic_Synchronous_Position, int32_t,
                          0x607A, 0, 0>
    CyclicSynchronousPositionMode;
typedef ModeForwardHelper<MotorBase::Cyclic_Synchronous_Velocity, int32_t,
                          0x60FF, 0, 0>
    CyclicSynchronousVelocityMode;
typedef ModeForwardHelper<MotorBase::Cyclic_Synchronous_Torque, int16_t, 0x6071,
                          0, 0>
...
typedef ModeForwardHelper<MotorBase::Velocity, int16_t, 0x6042, 0,
                          (1 << Command402::CW_Operation_mode_specific0) |
                              ...
                              (1 << Command402::CW_Operation_mode_specific2)>
    VelocityMode;
typedef ModeForwardHelper<MotorBase::Interpolated_Position, int32_t, 0x60C1,
...
public:
  Motor402(const std::string &name, ObjectStorageSharedPtr storage,
           const canopen::Settings &settings)
      : MotorBase(name), status_word_(0), control_word_(0),
        switching_state_(
            State402::InternalState(settings.get_optional<unsigned int>(
                "switching_state",
  ...
  virtual void registerDefaultModes(ObjectStorageSharedPtr storage) {
    ...
    registerMode<InterpolatedPositionMode>(MotorBase::Interpolated_Position,
                                           storage);
    registerMode<CyclicSynchronousPositionMode>(
        MotorBase::Cyclic_Synchronous_Position, storage);
  ...
  public:
    virtual MotorBaseSharedPtr allocate(const std::string &name,
