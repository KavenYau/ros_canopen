...
State402::InternalState State402::read(uint16_t sw) {
  ...
  default:
    LOG("Motor is currently in an unknown state: " << std::hex << state
                                                   << std::dec);
  }
  boost::mutex::scoped_lock lock(mutex_);
  if (new_state != state_) {
...
bool State402::waitForNewState(const time_point &abstime,
...
Command402::TransitionTable::TransitionTable() {
  ...
  Op shutdown((1 << CW_Quick_Stop) | (1 << CW_Enable_Voltage),
              (1 << CW_Fault_Reset) | (1 << CW_Switch_On));
...
bool Command402::setTransition(uint16_t &cw,
...
bool DefaultHomingMode::executeHoming(canopen::LayerStatus &status) {
  ...
  if (!cond_.wait_until(
          lock, prepare_time,
          masked_status_not_equal<MASK_Error | MASK_Reached, 0>(status_))) {
  ...
  if (!cond_.wait_until(
          lock, prepare_time,
          masked_status_not_equal<MASK_Error | MASK_Attained | MASK_Reached,
  ...
  if (!cond_.wait_until(
          lock, finish_time,
          masked_status_not_equal<MASK_Error | MASK_Attained, 0>(status_))) {
...
bool Motor402::switchMode(LayerStatus &status, uint16_t mode) {
  ...
  if (!switchState(status, switching_state_))
    return false;
  ...
  { // wait for switch
    ...
    if (monitor_mode_) {
      while (mode_id_ != mode &&
             mode_cond_.wait_until(lock, abstime) ==
                 boost::cv_status::no_timeout) {
      }
    } else {
      while (mode_id_ != mode && get_abs_time() < abstime) {
        boost::reverse_lock<boost::mutex::scoped_lock> reverse(
            lock);              // unlock inside loop
        op_mode_display_.get(); // poll
        boost::this_thread::sleep_for(
            boost::chrono::milliseconds(20)); // wait some time
      }
    }
...
bool Motor402::switchState(LayerStatus &status,
  ...
  while (state != target_state_) {
    ...
    if (!Command402::setTransition(control_word_, state, target_state_,
                                   &next)) {
...
bool Motor402::readState(LayerStatus &status, const LayerState &current_state) {
  ...
  boost::mutex::scoped_lock lock(mode_mutex_);
  uint16_t new_mode =
      monitor_mode_ ? op_mode_display_.get() : op_mode_display_.get_cached();
...
void Motor402::handleRead(LayerStatus &status,
...
void Motor402::handleWrite(LayerStatus &status,
                           const LayerState &current_state) {
  if (current_state > Off) {
    boost::mutex::scoped_lock lock(cw_mutex_);
...
void Motor402::handleDiag(LayerReport &report) {
  ...
  report.add("Ready to switch on",
             bool(sw & (1 << State402::SW_Ready_To_Switch_On)));
  report.add("Switched on", bool(sw & (1 << State402::SW_Switched_On)));
  report.add("Operation enabled",
             bool(sw & (1 << State402::SW_Operation_enabled)));
  report.add("Fault", bool(sw & (1 << State402::SW_Fault)));
  report.add("Voltage enabled", bool(sw & (1 << State402::SW_Voltage_enabled)));
  report.add("Quick stop (inactive when true)",
             bool(sw & (1 << State402::SW_Quick_stop)));
  report.add("Switch on disabled",
             bool(sw & (1 << State402::SW_Switch_on_disabled)));
  report.add("Homing/CALLS waring", bool(sw & (1 << State402::SW_Warning)));
  report.add("Homing/CALLS active",
             bool(sw & (1 << State402::SW_Manufacturer_specific0)));
  report.add("Remote", bool(sw & (1 << State402::SW_Remote)));
  report.add("Target Reached", bool(sw & (1 << State402::SW_Target_reached)));
  report.add("Internal limit active",
             bool(sw & (1 << State402::SW_Internal_limit)));
  report.add("Set-point acknowledge/Speed is 0",
             bool(sw & (1 << State402::SW_Operation_mode_specific0)));
  report.add("Following/Max. slippage error",
             bool(sw & (1 << State402::SW_Operation_mode_specific1)));
  report.add("Event has occured",
             bool(sw & (1 << State402::SW_Manufacturer_specific1)));
  report.add("Axis is on",
             bool(sw & (1 << State402::SW_Manufacturer_specific2)));
