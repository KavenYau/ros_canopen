...
hardware_interface::hardware_interface_ret_t RobotLayer::write() {
...
void RobotLayer::stopControllers(const std::vector<std::string> controllers) {
  ...
  // boost::thread
  // call(std::bind(ros::service::call<controller_manager_msgs::SwitchController>,
...
RobotLayer::RobotLayer(rclcpp::Logger logger)
    : LayerGroupNoDiag<HandleLayerBase>("RobotLayer"), first_init_(true),
      ros_logger_(logger) {
  // ros_logger_ = logger;
...
void RobotLayer::handleInit(LayerStatus &status) {
  if (first_init_) {
    for (HandleMap::iterator it = handles_.begin(); it != handles_.end();
      ...
      // }
      //
      // bool has_joint_limits = joint_limits_interface::getJointLimits(joint,
      // limits);
      //
      // has_joint_limits = joint_limits_interface::getJointLimits(it->first,
      // nh_, limits) || has_joint_limits;
      //
      // bool has_soft_limits = has_joint_limits &&
      // joint_limits_interface::getSoftJointLimits(joint, soft_limits);
      //
      // if(!has_joint_limits){
      ...
      // &soft_limits : 0);
      // it->second->registerHandle(vel_interface_, limits, has_soft_limits ?
      // &soft_limits : 0);
      // it->second->registerHandle(eff_interface_, limits, has_soft_limits ?
      // &soft_limits : 0);

      // RCLCPP_INFO(ros_logger_, "motor node: %u",
      // it->second->motor_->getMode());
    }
    first_init_ = false;
  }
...
// bool RobotLayer::prepareSwitch(const
...
//     (void) &hardware_interface::RobotHW::prepareSwitch; // please upgrade to
//     ros_control/contoller_manager 0.9.4 or newer
//
//     // stop handles
//     for (std::list<hardware_interface::ControllerInfo>::const_iterator
//     controller_it = stop_list.begin(); controller_it != stop_list.end();
//     ++controller_it){
//
//         if(switch_map_.find(controller_it->name) == switch_map_.end()){
//             ROS_ERROR_STREAM(controller_it->name << " was not started
//             before");
//             return false;
//         }
//     }
...
//             for
//             (std::vector<hardware_interface::InterfaceResources>::const_iterator
//             cres_it = controller_it->claimed_resources.begin(); cres_it !=
//             controller_it->claimed_resources.end(); ++cres_it){
//                 for (std::set<std::string>::const_iterator res_it =
//                 cres_it->resources.begin(); res_it !=
//                 cres_it->resources.end(); ++res_it){
//                     claimed_interfaces.insert(cres_it->hardware_interface);
//                     if(!ml.hasModes()){
//                         ROS_ERROR_STREAM("Please set required_drive_mode(s)
//                         for controller " << controller_it->name);
//                         return false;
//                     }
//                     if(claimed_interfaces.size() > 1 && !ml.hasMixedModes()){
//                         ROS_ERROR_STREAM("controller "<< controller_it->name
//                         << " has mixed interfaces, please set
//                         required_drive_modes.");
//                         return false;
//                     }
//
//                     std::unordered_map< std::string, HandleLayerBaseSharedPtr
//                     >::const_iterator h_it = handles_.find(*res_it);
//
...
//                         ROS_ERROR_STREAM("could not determine drive mode for
//                         " << joint);
//                         return false;
//                     }
//
//                     if(g_interface_mapping.hasConflict(cres_it->hardware_interface,
//                     sd.mode)){
//                         ROS_ERROR_STREAM(cres_it->hardware_interface << "
//                         cannot be provided in mode " << sd.mode);
//                         return false;
//                     }
//
//                     HandleLayerBase::CanSwitchResult res =
//                     h_it->second->canSwitch(sd.mode);
//
//                     switch(res){
//                         case HandleLayerBase::NotSupported:
//                             ROS_ERROR_STREAM("Mode " << sd.mode << " is not
//                             available for " << joint);
//                             return false;
//                         case HandleLayerBase::NotReadyToSwitch:
//                             ROS_ERROR_STREAM(joint << " is not ready to
//                             switch mode");
//                             return false;
...
//         for(RobotLayer::SwitchContainer::iterator it = to_switch.begin(); it
//         != to_switch.end(); ++it){
...
//                 ROS_ERROR_STREAM("Could not switch one joint for " <<
//                 controller_it->name << ", will stop all related joints and
//                 the controller.");
//                 for(RobotLayer::SwitchContainer::iterator stop_it =
//                 to_switch.begin(); stop_it != to_switch.end(); ++stop_it){
...
//         }
//     }
//     for(std::unordered_set<HandleLayerBaseSharedPtr >::iterator it =
//     to_stop.begin(); it != to_stop.end(); ++it){
//         (*it)->switchMode(MotorBase::No_Mode);
//     }
//     if(!failed_controllers.empty()){
//         stopControllers(failed_controllers);
//         // will not return false here since this would prevent other
//         controllers to be started and therefore lead to an inconsistent state
//     }
//
//     return true;
// }
...
//         try{
//             SwitchContainer &to_switch = switch_map_.at(controller_it->name);
//             for(RobotLayer::SwitchContainer::iterator it = to_switch.begin();
//             it != to_switch.end(); ++it){
//                 if(!it->handle->forwardForMode(it->mode)){
//                     failed_controllers.push_back(controller_it->name);
//                     ROS_ERROR_STREAM("Could not switch one joint for " <<
//                     controller_it->name << ", will stop all related joints
//                     and the controller.");
//                     for(RobotLayer::SwitchContainer::iterator stop_it =
//                     to_switch.begin(); stop_it != to_switch.end();
//                     ++stop_it){
...
//             ROS_ERROR_STREAM("Conttroller " << controller_it->name << "not
//             found, will stop it");
//             failed_controllers.push_back(controller_it->name);
//         }
//     }
